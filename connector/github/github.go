...
func (c *githubConnector) listGroups(ctx context.Context, client *http.Client, userName string) (groups []string, err error) {
	for _, org := range c.orgs {
		...
		// User is in at least one org. Filter out teams not in 'teams' list in
		// config. Otherwise include all teams in claim.
		if len(org.Teams) > 0 {
...
func filterTeams(userTeams, teamRestrictions []string) (teams []string) {
	teamFilter := make(map[string]struct{})
	for _, team := range teamRestrictions {
